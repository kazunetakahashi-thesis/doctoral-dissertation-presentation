...
$I$ が $(\text{PS})_c$ 条件を充たすとは， $I$ の任意の $(\text{PS})_c$ 列が precompact (以下，相対コンパクトの意で使う) であることをいう．
...
どの手法も，大体 [[Brézis-Nirenberg '83] 型の手法](b_n.html) になる．以下だけ覚える：

- 種々の評価は， *「定数部分」＋「残余項」* の形になる．
- mountain pass level や $(\text{PS})_ c$ 条件は， **定数部分は「正確な値」が必要** になる．例えば $S^{N/2} / N$ である．
...
$\alpha > 0$, $\lambda > \lambda_1'$ は定数である．ここで $\lambda_1'$ は，次の Dirichlet 固有値問題の第 $1$ 固有値とする：$-\Delta \phi = \lambda \phi \text{ in } \Omega$. ここで $\alpha > 0$ が十分小さいとき， sign-changing solution の存在が，それぞれ以下の条件で示されている．
...
\[
  \begin{align}
    u_{\epsilon, l}(x) &= \frac{\xi_l(x)}{\left( \epsilon + \left\lvert x - x_ l \right\rvert^2 \right)^{(N - 2)/2}}, \\
...
Kirchhoff 方程式は，もともと時間発展する方程式．例えば string の vibration と関係する． $p = 4$ とした
...
- $N = 3$ の場合
  - $4 < q < 2^* = 6$ の場合： [Naimen '14 NoDEA] で $\lambda > 0$ に対して解が存在することが示されている．
    - 今回我々が参考にする手法．
  - $q = 4$ の場合： [Naimen '14 NoDEA] で結果が出ている．
    - Brézis-Nirenberg の $N = 3$ の type の議論をする．
...
\end{multline}
\]
が得られるが，左辺は全ての項が $O(T_ \epsilon^2 A_ \epsilon  - \eta_ 0)$ ，右辺は $O(A_ \epsilon - S)$ である．さらに $g(T_ \epsilon^2 A_ \epsilon) < 0$ であることも鑑みると， $T_ \epsilon^2 A_ \epsilon  - \eta_ 0 > 0$ もわかる．よって (8) が得られる．
