...
- この文書は、審査会終了後インターネット上で公開する。
  - 色弱な方も考慮し、全てのコンテンツは白黒。強調は太字と斜体のみを使う。
  - 参考文献の正式名称は掲げないが、少なくとも博士論文の Reference には引いてある。 Reference は同時に公開する。
- 博士論文をお読みくださった方に向けて発表を行う。
  - 種々の事情で論文に *書けなかった内容* を述べ、補足説明、意図の説明をする。
  - 「この分野に深い理解をしている人が、拡張したくなる」内容を目指す。
...
- 例えば $n$ 個の配列を merge sort する際の時間計算量は $O(n \log n)$ であるが、別にこれを $O(n^2)$ と言っても $O(2^n)$ と言っても、本来は間違いではない。上界を表す記号であるから。
  - 下界は $\Theta$ を使う。それらがぴったり一致する場合は $\Omega$ を用いるのが一般的である。
...
- Sobolev 臨界指数を持つ Henon 方程式の解の存在を示した。
  - 移動する Talenti 関数の積分の評価を正確に行った。 $N \geq 3$ で $\epsilon$-order を決定 or 残余項を評価した。
  - 境界で項が消える非自明な例も含まれている。
- Kirchhoff 方程式に、その手法を適用した。
  - 応用する際に non-local term を拡張してから適用した。
- 今回開発した手法を適用すると、既存の Sobolev 臨界指数を持つ方程式を Henon 型に拡張しても、存在する条件がほとんど失われないと見込まれる。
...
実は、このような手法は、よく調べると [Zeng-Tang '16] で導入されていた。こちらは $\alpha = 0$, $a > 0$ に止まっている。しかし私は [Naimen '14 NoDEA] の直接拡張に成功したので、 $a = 0$, $b > 0$ の場合が含まれている。これは Henon 型との相性も良い。
